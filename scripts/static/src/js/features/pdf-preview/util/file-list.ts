  ...
  if (outputFiles) {
    ...
    for (const file of outputFiles.values()) {
      if (!ignoreFiles.includes(file.path)) {
        ...
        // Offline build: blob: URLs don't accept query strings (the browser
        // looks them up by UUID and ignores everything after `?`). Skip the
        // append for our locally-served compile output.
        if (
          queryString.length &&
          !(file.url?.startsWith('blob:') || file.url?.startsWith('http'))
        ) {
