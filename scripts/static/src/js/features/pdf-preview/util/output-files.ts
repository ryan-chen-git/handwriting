...
export function handleOutputFiles(
  ...
  // Offline build: blob: URLs don't accept query strings (the browser looks
  // them up by UUID). Pass through unchanged.
  const isBlob = outputFile.url?.startsWith('blob:')
  outputFile.pdfUrl = isBlob
    ? buildURL(outputFile, data.pdfDownloadDomain)
    : `${buildURL(outputFile, data.pdfDownloadDomain)}?${params}`

  if (data.fromCache) {
    outputFile.pdfDownloadUrl = outputFile.downloadURL
  } else if (isBlob) {
    // For blob: URLs the same blob is the download target.
...
function buildURL(file: PDFFile, pdfDownloadDomain?: string): string {
  // Offline build: our compiler returns a blob: URL — it's already a complete
  // URL, so don't prepend an origin or download domain.
