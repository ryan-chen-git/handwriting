...
export default class DocumentCompiler {
  ...
  async compile(options: CompileOptions = {}) {
    ...
    try {
      ...
      // Offline build: bypass upstream's /project/:id/compile (which talks
      // to CLSI). Read the current editor text from the window-attached
      ...
      let localJson: { ok: boolean; pdf?: string; log?: string }
      if (!latex.trim()) {
        localJson = { ok: false, log: '' }
      } else {
      ...
      const logBlobUrl = URL.createObjectURL(
        new Blob([localJson.log ?? ''], { type: 'text/plain' })
      )
      if (localJson.ok && localJson.pdf) {
        // The Python server returns { ok, pdf: <base64>, log }. Decode the
        // base64 PDF + log into blob: URLs the upstream PDF viewer + log
        // parser can fetch directly.
        const pdfBytes = Uint8Array.from(atob(localJson.pdf), c => c.charCodeAt(0))
        ...
        data = {
          status: 'success',
          outputFiles: [
            { path: 'output.pdf', url: pdfBlobUrl, build: buildId, type: 'pdf' },
            { path: 'output.log', url: logBlobUrl, build: buildId, type: 'log' },
          ],
          clsiServerId: 'local',
          compileGroup: 'standard',
          pdfDownloadDomain: '',
          rootDocId,
        } as unknown as CompileResponseData
      } else {
        data = {
          status: 'failure',
          outputFiles: [
            { path: 'output.log', url: logBlobUrl, build: String(Date.now()), type: 'log' },
          ],
          clsiServerId: 'local',
          compileGroup: 'standard',
          rootDocId,
