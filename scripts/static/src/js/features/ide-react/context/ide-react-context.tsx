...
export const IdeReactProvider: FC<React.PropsWithChildren> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Offline build: the real-time service that fires `joinProjectResponse`
    // is stubbed. Synthesize a response from getMeta('ol-project') so the
    // file tree + editor can mount.
    const stubProject = getMeta('ol-project')
    if (stubProject) {
      handleJoinProjectResponse({
        project: stubProject as any,
