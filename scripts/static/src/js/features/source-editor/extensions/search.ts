...
// `togglePanel` is an internal effect not part of @codemirror/search's public
// API. Upstream forks the package; we drop the reset-on-close code path.
import {
  Decoration,
  EditorView,
  KeyBinding,
...
const storedSelectionState = StateField.define<EditorSelection | null>({
  ...
  update(value, tr) {
    ...
    for (const effect of tr.effects) {
      ...
      // upstream also reset on togglePanel(false); skipped here since the
      // togglePanel effect isn't exported by @codemirror/search publicly.
    }
